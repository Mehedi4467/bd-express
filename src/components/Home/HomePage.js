  ...
  const verifyTrackingId = async () => {
    ...
    if (verifyTI?.status === "Accepted") {
      ...
      if (verifyTI?.data?.status === 0) {
        ...
        if (
          (z?.data?.primary_data?.extra_work?.length === 1 &&
            z?.data?.primary_data?.extra_work[0]?.name === "Payment") ||
          z?.data?.primary_data?.extra_work?.status === "complete"
        ) {
          setExtraWork(false);
          navigation.navigate("update", {
            trackingID: verifyTI?.data?.tracking_id,
            BookingID: verifyTI?.data?.booking
          });
          setCode("");
          setInputCode("");
        } else {
          setExtraWork({
            data: z?.data?.primary_data?.extra_work?.work,
  ...
  return (
    <View style={styles.container}>
      ...
      {trackingLoading ? (
        ...
        <Text>{trackingMsg}</Text>
      ) : (
        ""
      )}
      {inputCode &&
      trackingVerifyData?.status === "Accepted" &&
      trackingVerifyData?.data?.status == 0 &&
      createBooking ? (
        <>
        ...
        trackingVerifyData?.status === "Accepted" &&
        trackingVerifyData?.data?.status == 2 &&
        createBooking ? (
        <>
          <TouchableOpacity style={styles.button} onPress={handlePress}>
            <Text style={styles.buttonText}>Create Booking</Text>
